...
authornote: |
  ...
  JRN and MM are supported by ONR N00014-23-1-2792. MS was funded by the Deutsche Forschungsgemeinschaft (DFG, German Research Foundation) -- GRK 2277 -- Projektnummer 310365261.
...
```{r reliability,fig.cap="Reliability as a function of trial size ($L$) for different signal-to-noise values ($\\gamma$). Higher values of $\\gamma$ lead to higher reliability for fewer trials."}

reliability=function(g2,L) g2/(g2+2/L) 
attenuation2=function(gamma1,gamma2,L){
  sqrt(rel(gamma1^2,L)*rel(gamma2^2,L))
}

L.val=function(coef,power) coef*10^power
L=as.vector(outer(1:9,0:4, L.val))
g2=c(.05,.1,.2,.3,.4,.5,1)^2
g=c(.05,.1,.2,.3,.4,.5,1)

r=outer(g2,L,reliability)

cols = c("indianred3", "#EE7942", "#548B54", "cyan2", "#3A5FCD", "#FF69B4", "black" )
matplot(log10(L), t(r), type = "l", lwd = 2, lty = 1, col = cols,
        axes = FALSE, xlab = "Trial Size", ylab = "Reliability")
axis(2)
axis(1,at=0:4,label=10^(0:4))
axis(1,at=log10(L),lab=NA)
labs = lapply(g, function(g) bquote(gamma == .(g)))
legend("bottomright", 
       legend = labs,
       col = cols, 
       lwd = 2, 
...
## Signal-To-Noise Task Goodness Measure

The parameter $\gamma^2$ is defined in (\ref{gamma2}) as the ratio of between-individual variance and residual trial noise.  The numerator, $\sigma^2$, serves as the *signal* in that if it is large people are easily discriminable.  The denominator, $\tau^2$, is *noise*.  Hence, $\gamma^2$ is the signal-to-noise ratio of an experiment, and it, unlike reliability, can be stated without recourse to trial size [@Rouder.etal.2023].

@Rouder.Mehrvarz.2024 advocate $\gamma^2$ as a task-goodness measure.  They ask researchers to report these values and use them for planning trial sizes.  They note that (\ref{eq.reliability}) implies:
$$
...
@Rouder.Mehrvarz.2024 provide the following method-of-moments estimator for a contrast:
$$
...
@McGraw.Wong.1996 note that ICC(1) is an estimate of $\kappa$:
$$
...
@McGraw.Wong.1996] recommend computing ICC(1) from a one-way ANOVA as 
$$
...
# Low Reliability and Misplaced Certainty in Low Correlations
...
makeData=function(I,L=100){
  ...
  tTheta=rmvnorm(I,c(70,70),
                 matrix(ncol=2,c(1,.5,.5,1)*25^2))
  y=rnorm(I*L*4,tAlpha[sub]+(cond-1.5)*tTheta[cbind(sub,task)],175)
  out=data.frame(sub,task,cond,y)
  return(out)}
...
Correlations serve as the main target when using experiments to understand individual differences.  And when estimating them,  our target should be the true correlation among individuals across experimental tasks as if there were an unlimited number of trials, that is what is denoted $\rho$ rather than $\rho^*$ in the preceding development.  In estimating $\rho$, we should strive for two desiderata.  The first is **consistency** in estimation with respect to increasing numbers of individuals.  As this number becomes larger, the estimate should become more accurate and, in the limit, achieve the true value with certainty.  The second desideratum is an **accurate assessment of uncertainty.** The associated error bars or confidence intervals should accurately quantify the uncertainty in estimation.  Sample correlations fail dramatically to meet these desiderata.  One is too certain in an attenuated value.  
...
ybar=tapply(dat$y,list(dat$sub,dat$task,dat$cond),mean)
score=ybar[,,2]-ybar[,,1]
cOut=cor.test(score[,1],score[,2])
emp=c(cOut$estimate,cOut$conf.int)

out=runMod(dat,M=5000)
```
```{r}
pEffect=out$BUGSoutput$sims.list$effect
mn=apply(pEffect,2:3,mean)
lo=apply(pEffect,2:3,quantile,p=.025)
hi=apply(pEffect,2:3,quantile,p=.975)

s2Inv=out$BUGSoutput$sims.list$Sigma_inv
M=dim(s2Inv)[1]
p=s2Inv
for (m in 1:M){
  p[m,,]=cov2cor(solve(s2Inv[m,,]))
}

ci=quantile(p[,1,2],p=c(.025,.975))
```

```{r example,fig.cap="Model outputs.  A. The points (plusses) are observed individual effects ordered from smallest to largest for one of the two tasks.  The solid line is the corresponding model point estimate; the shaded area is the corresponding 95% credible interval.  There is a large degree of regularization indicating that much of the variability in the observed scores is due to trial noise.  B. The histogram shows the posterior distribution of the correlation coefficient.  The thick vertical line is the true value, and it is well within the posterior.  The red point with confidence intervals is the observed correlation.  It is attenuated and the CIs do not cover the true value."}
par(mfrow=c(1,2))
o=order(score[,1])
plot(1:I,score[,1],typ='n',axes=F,ylab="Effect (ms)",xlab="Individual")
axis(1,at=c(1,I))
axis(2)
polygon(c(1:I,I:1),c(lo[o,1],rev(hi[o,1])),col='lightblue',border=NA)
lines(1:I,mn[o,1],col='darkblue')
points(1:I,score[o,1],pch=3)

plot(typ='n',xlim=c(-1.05,1.05),ylim=c(0,2.2),0:1,0:1,axes=F,
     ylab="Density",xlab="Correlation Coefficient")
x=seq(-1,1,.01)
y=dunif(x,-1,1)
hist(p[,1,2],prob=T,main="",add=T,col="lightblue",breaks=20,border="lightblue")
lines(c(-1,x,1),c(0,y,0))
axis(1)
axis(2)
abline(v=.5,lwd=3)
arrows(emp[2],2,emp[3],2,code=3,angle=90,col='darkred',length=.1)
points(emp[1],2,pch=19,col='darkred',cex=1.2)
```


How to interpret this posterior uncertainty?  The 95% credible interval from the posterior is [`r ci[1]`,`r ci[2]`], the width of which is  `r diff(ci)`.  We can compare this to the smaller interval from the correlation among observed contrasts, which is only `r diff(emp)[2]` in width.  The width of this interval reflects only the value of the correlation and the number of individuals $I$.  In this sense, it reflects to large degree the uncertainty from a finite sample of individuals.  Indeed, if there were a great many trials, the posterior uncertainty in the hierarchical model would approach this width.  The additional width in the hierarchical posterior reflects the uncertainty from a finite number of trials.  This is the appropriate measure researchers should convey when assessing the importance of correlations---it reflects the true limitations of the design.

# Reliability Is No Longer Important For Interpreting Experiments
...
# Reliability in Planning Experiments
...
The results of the simulation are shown in Figure \ref{fig:sims}.  The left panel shows the ratio $w/s$, termed the *Width Ratio* as a function of $1/a$, termed the *Expansion Factor*.  The median is shown as the black line, and indeed, it is very close to the diagonal.  The 10% and 90% of the ratios are also shown, and these deviate in a reasonable fashion from the diagonal.

```{r sims, fig.cap="Planning for trial noise.  A. Width ratio as a function of expansion factor.  Width ratio is expansion of uncertainty when taking into account trial noise and is measured as the ratio of the posterior credible interval width to the sample confidence interval width ($w/s$).  Exapansion factors are the reciprocal of the geometric mean of task reliability ($1/a$).  The figure shows the case for $\\gamma=.2$ across a variety of trial sizes ($L$) and sample sizes ($I$).  Gray dots are simulation replicates; the solid lines mark the 10th, 50th, and 90th percentiles across replicates. B. Tradeoff of trial size $L$ and $I$ needed to acheive a target credible interval with of $.2$ in value for different  signal-to-noise ratio values ($\\gamma$).",fig.width=10, fig.height=5.5}
source("funs.R")
par(mfrow = c(1,2))
plotCIwidth()
plotIvsL()
...
```{r overall, echo=FALSE, out.width='100%', fig.align='center',fig.cap="Analysis begins with measures of task goodness that are invariant to trial size $L$ and sample size $I$.  Suitable measures are signal-to-noise and ICC(1).  Reliability and attenuation factors reflect task goodness and trial size $L$ but not sample size $I$.  Uncertainty on correlations reflects reliability and the number of individuals $I$.  In this sense, reliability sits between task goodness and correlation precision---incorporating trial size but not sample size."}
knitr::include_graphics("planning.pdf")
```
